        ...
        <DropdownMenu>
          ...
          {signedIn ? (
            <DropdownMenuContent className="border border-white/20 p-7 text-base bg-black/80 backdrop-blur-sm rounded-md text-white shadow-lg">
              <DropdownMenuItem
                className="p-2 hover:text-orange-400 hover:bg-white/10 rounded cursor-pointer  text-center mb-2 bg-neutral-800"
                onClick={logout}
              >
                {logoutLoader ? (
                  <CircleLoader color="red" loading={logoutLoader} size={20} />
                ) : (
                  "Log Out"
                )}
              </DropdownMenuItem>
